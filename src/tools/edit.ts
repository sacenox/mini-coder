...
const EditInput = z.object({
  ...
  newString: z
    .string()
    .describe("String to replace oldString with. Pass empty string to delete oldString."),
...
export const editTool: ToolDef<EditInput, EditOutput> = {
  ...
  execute: async (input) => {
    ...
    if (firstIdx === -1) {
      throw new Error(
        `oldString not found in "${relPath}". ` +
    ...
    if (secondIdx !== -1) {
      throw new Error(
        `oldString matches multiple locations in "${relPath}". ` +
