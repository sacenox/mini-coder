...
let _stdinReader: any | null = null;
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        process.stdin.on("data", (chunk: Buffer) => {
          try { controller.enqueue(new Uint8Array(chunk)); } catch { /* ignore if closed */ }
        });
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function readKey(reader: any): Promise<string> {
...
export async function readline(opts: {
  ...
  function renderPrompt(): void {
    ...
    const display = buf.length > cols - PROMPT_RAW_LEN - 2
  ...
  function renderSearchPrompt(): void {
    process.stdout.write(
      `${CLEAR_LINE}${c.cyan("search:")} ${searchQuery}█`
    );
  }
    ...
    while (true) {
      ...
      if (raw.startsWith(ESC)) {
        ...
        if (raw === `${CSI}A` || raw === `${ESC}[A`) {
          ...
          if (histIdx > 0) { histIdx--; applyHistory(); }
          continue;
        }
        if (raw === `${CSI}B` || raw === `${ESC}[B`) {
          // Down arrow — history next
          if (histIdx < history.length) { histIdx++; applyHistory(); }
          continue;
        }
        if (raw === `${CSI}C` || raw === `${ESC}[C`) {
          // Right arrow
          if (cursor < buf.length) { cursor++; renderPrompt(); }
          continue;
        }
        if (raw === `${CSI}D` || raw === `${ESC}[D`) {
          // Left arrow
          if (cursor > 0) { cursor--; renderPrompt(); }
          continue;
        }
