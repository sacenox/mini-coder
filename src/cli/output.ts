...
function toolCallLine(name: string, args: unknown): string {
  ...
  if (name === "glob") {
    return `${G.search} ${c.dim("glob")} ${c.bold(String(a["pattern"] ?? ""))}${a["cwd"] ? c.dim(` in ${a["cwd"]}`) : ""}`;
  }
  if (name === "grep") {
    const flags = [
    ...
    return `${G.search} ${c.dim("grep")} ${c.bold(String(a["pattern"] ?? ""))}${flags ? c.dim(`  ${flags}`) : ""}`;
  }
  if (name === "read") {
  ...
  if (name === "shell") {
    ...
    return `${G.run} ${c.dim("$")} ${cmd.length > 72 ? cmd.slice(0, 69) + "…" : cmd}`;
  }
  if (name === "subagent") {
    const prompt = String(a["prompt"] ?? "");
    return `${G.agent} ${c.dim("subagent")} ${c.dim(prompt.length > 60 ? prompt.slice(0, 57) + "…" : prompt)}`;
  }
...
export function renderToolResult(
  ...
  if (toolName === "glob") {
    ...
    const files = r.files;
    if (files.length === 0) { writeln(`    ${G.info} ${c.dim("no matches")}`); return; }
    ...
    for (const f of show) {
      const rel = prefix && f.startsWith(prefix) ? c.dim(prefix) + f.slice(prefix.length) : f;
      writeln(`    ${c.dim("·")} ${rel}`);
    }
  ...
  if (toolName === "grep") {
    const r = result as {
      matches: Array<{ file: string; line: number; text: string; context?: Array<{ line: number; text: string; isMatch: boolean }> }>;
      truncated: boolean;
    };
    if (r.matches.length === 0) { writeln(`    ${G.info} ${c.dim("no matches")}`); return; }
    ...
    for (const m of r.matches) {
      if (shown >= 10) break;
      const key = m.file;
      const fileLabel = seen.has(key) ? c.dim(" ".repeat(m.file.length + 1)) : c.dim(m.file + ":");
      seen.add(key);
  ...
  if (toolName === "read") {
    const r = result as {
      path: string; startLine: number; endLine: number; totalLines: number; truncated: boolean;
    };
    const range = r.startLine === 1 && r.endLine === r.totalLines
      ? `${r.totalLines} lines`
      : `lines ${r.startLine}–${r.endLine} of ${r.totalLines}`;
    writeln(`    ${G.info} ${c.dim(`${r.path}  ${range}${r.truncated ? "  (truncated)" : ""}`)}`);
    return;
  }
  ...
  if (toolName === "shell") {
    ...
    if (outLines.length > 20) writeln(`    ${c.dim(`│ … +${outLines.length - 20} lines`)}`);

    // Show stderr (up to 8 lines), only if non-empty
    for (const line of errLines.slice(0, 8)) {
      if (line.trim()) writeln(`    ${c.red("│")} ${c.dim(line)}`);
    }
    if (errLines.length > 8) writeln(`    ${c.red(`│ … +${errLines.length - 8} lines`)}`);
    return;
  }

  if (toolName === "subagent") {
    // subagent result: { result: string, inputTokens, outputTokens }
    const r = result as { result?: string; inputTokens?: number; outputTokens?: number };
    if (r.result) {
      const lines = r.result.split("\n");
      const preview = lines.slice(0, 8);
      for (const line of preview) writeln(`    ${c.dim("│")} ${line}`);
      if (lines.length > 8) writeln(`    ${c.dim(`│ … +${lines.length - 8} lines`)}`);
    }
  ...
  if (toolName.startsWith("mcp_")) {
    ...
    for (const block of (content as Array<{ type?: string; text?: string }>).slice(0, 5)) {
...
export async function renderTurn(
  ...
  spinner: Spinner
): Promise<{ inputTokens: number; outputTokens: number; newMessages: import("../llm-api/types.ts").Message[] }> {
  let inText = false;
  let inputTokens = 0;
  let outputTokens = 0;
  let newMessages: import("../llm-api/types.ts").Message[] = [];
  ...
  for await (const event of events) {
    switch (event.type) {
      ...
      case "tool-call-start": {
        if (inText) { writeln(); inText = false; }
        spinner.stop();
...
export function renderStatusBar(opts: {
  ...
  if (opts.inputTokens > 0 || opts.outputTokens > 0) {
    right.push(c.dim(`↑${fmtTokens(opts.inputTokens)} ↓${fmtTokens(opts.outputTokens)}`));
  }
  ...
  const out = visible.length > cols
    ? truncateAnsi(full, cols - 1)
    : full;
