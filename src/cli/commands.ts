...
import { writeln, renderInfo, renderError, PREFIX } from "./output.ts";
import { availableProviders, fetchAvailableModels } from "../llm-api/providers.ts";
import { listMcpServers, upsertMcpServer, deleteMcpServer } from "../session/db.ts";
...
export interface CommandContext {
  ...
  runSubagent: (prompt: string, model?: string) => Promise<{ result: string; inputTokens: number; outputTokens: number }>;
  cwd: string;
}
...
async function handleModel(ctx: CommandContext, args: string): Promise<void> {
  if (args) {
    ...
    if (!args.includes("/")) {
      ...
      if (match) {
        ...
        writeln(`${PREFIX.error} unknown model ${c.cyan(args)}  ${c.dim("— run /models for the full list")}`);
        return;
      }
    }
  ...
  if (models.length === 0) {
    ...
    writeln(c.dim("  Set OPENCODE_API_KEY for Zen, or start Ollama for local models."));
    return;
  }
  ...
  for (const [provider, list] of byProvider) {
    ...
    for (const m of list) {
      ...
      const ctxTag = m.context ? c.dim(` ${Math.round(m.context / 1000)}k`) : "";
  ...
  writeln();
  writeln(c.dim(`  /model <id>  to switch  ·  e.g. /model zen/claude-sonnet-4-6`));
}

function handlePlan(ctx: CommandContext): void {
  ctx.setPlanMode(!ctx.planMode);
  if (ctx.planMode) {
    writeln(`${PREFIX.info} ${c.yellow("plan mode")} ${c.dim("— agent will describe without running tools")}`);
  } else {
  ...
  switch (sub) {
    case "list": {
      ...
      if (servers.length === 0) {
        ...
        writeln(c.dim("  /mcp add <name> http <url>  ·  /mcp add <name> stdio <cmd> [args...]"));
        return;
      }
      writeln();
      for (const s of servers) {
        const detail = s.url ? c.dim(`  ${s.url}`) : s.command ? c.dim(`  ${s.command}`) : "";
        writeln(`  ${c.yellow("⚙")} ${c.bold(s.name)}  ${c.dim(s.transport)}${detail}`);
      }
      return;
    }
    ...
    case "add": {
      ...
      if (transport === "http") {
        upsertMcpServer({ name, transport, url: rest[0]!, command: null, args: null, env: null });
      } else if (transport === "stdio") {
        const [command, ...cmdArgs] = rest;
        upsertMcpServer({
          name, transport, url: null,
          command: command!,
          args: cmdArgs.length ? JSON.stringify(cmdArgs) : null,
          env: null,
        });
      } else {
        writeln(c.red(`  unknown transport: ${transport}  (use http or stdio)`));
        return;
      }
      // Connect immediately so tools are available in this session
      try {
        await ctx.connectMcpServer(name);
        writeln(`${PREFIX.success} mcp server ${c.cyan(name)} added and connected`);
      } catch (e) {
        writeln(`${PREFIX.success} mcp server ${c.cyan(name)} saved  ${c.dim("(connection failed: " + String(e) + ")")}`);
      }
      return;
    }
...
async function handleReview(ctx: CommandContext, args: string): Promise<void> {
  ...
  writeln(`${PREFIX.info} ${c.cyan("review")} ${c.dim("— spawning review subagent…")}`);
  writeln();
...
function handleHelp(): void {
  ...
  writeln();
  writeln(`  ${c.green("@file".padEnd(26))} ${c.dim("inject file contents into prompt (Tab to complete)")}`);
  writeln(`  ${c.green("!cmd".padEnd(26))} ${c.dim("run shell command, output added as context")}`);
  writeln();
  writeln(`  ${c.dim("ctrl+c")}  cancel  ${c.dim("·")}  ${c.dim("ctrl+d")}  exit  ${c.dim("·")}  ${c.dim("ctrl+r")}  history search  ${c.dim("·")}  ${c.dim("↑↓")}  history`);
  writeln();
}
...
export async function handleCommand(
  ...
  switch (command.toLowerCase()) {
    ...
    default:
      writeln(`${PREFIX.error} unknown: /${command}  ${c.dim("— /help for commands")}`);
