...
import { runTurn, userMessage } from "../llm-api/turn.ts";
import {
  writeln,
  renderTurn,
  renderError,
...
import { readline } from "../cli/input.ts";
import { handleCommand, type CommandContext } from "../cli/commands.ts";
...
function dbMessagesToCore(messages: Message[]): any[] {
  return messages.map((m) => {
    ...
    if (Array.isArray(m.content)) {
      ...
      if (textParts.length > 0) return { role: m.role, content: textParts };
...
export async function runAgent(opts: AgentOptions): Promise<void> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const coreHistory: any[] = dbMessagesToCore(session.messages);
    ...
    switch (input.type) {
      ...
      case "shell": {
        ...
        if (out) {
          ...
          coreHistory.push({ role: "user", content: msg.content });
        }
        continue;
      }
  ...
  async function processUserInput(text: string): Promise<void> {
    ...
    if (newMessages.length > 0) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const dbMsgs: Message[] = newMessages.map((m: any) => {
        if (typeof m.content === "string") {
        ...
        if (Array.isArray(m.content)) {
          ...
          const text = (m.content as any[])
            ...
            .join("");
          return { role: m.role as Message["role"], content: text };
        }
        return { role: m.role as Message["role"], content: String(m.content) };
      });
