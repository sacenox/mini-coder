...
When the effective startup settings include `mcp.servers`, mini-coder keeps those Streamable HTTP MCP endpoints in runtime state, connects enabled ones when needed, and imports their advertised tools.
...
2. **Build context** — construct a pi-ai `Context`: the system prompt (see [System prompt](#system-prompt)), the current model-visible history, and the registered tool definitions. Before each request, mini-coder first rebuilds that model-visible history from any persisted compaction summaries plus the remaining uncompacted raw messages. It estimates context pressure from the same model-bound view of history that will actually be sent, including any image-compatibility replacements for unsupported or over-budget images, so stripped images do not force unnecessary compaction. If that estimated next-request context pressure is at or above 90% of the active model window, mini-coder compacts the oldest ~40% of still-compactable messages with a separate summarization call that explicitly mentions the current user request. The resulting synthetic summary replaces that older prefix in future model context, includes a `<system-message>` reminder with the SQLite DB path and session id for raw-history recovery, records the hidden summary-call usage for later session stats, and previous compaction summaries are never compacted again. After any needed compaction, mini-coder may also apply a narrow image-compatibility pass to the actual request: if the current model does not support image input, or if the session has accumulated more than a small conservative image budget, image blocks are replaced with explicit text placeholders instead of being sent verbatim. The prompt context snapshot is loaded once at startup or another explicit reload boundary and then reused across turns so provider prompt caching stays effective. It is refreshed only at boundaries such as `/new` or CWD change.
...
**Context limit** — mini-coder automatically compacts conversation history when the estimated next-request context usage for the actual model-bound request reaches 90% of the active model window. That estimate reflects any image placeholders the request will send, so unsupported or over-budget images do not cause avoidable lossy compaction. Compaction summarizes the oldest ~40% of still-uncompacted model-visible messages with a separate model call that is anchored to the current user request, stores that synthetic summary separately from the raw transcript along with the hidden summary-call usage, and rebuilds future model context as `older summaries + remaining raw tail`. The raw transcript stays in the SQLite message log so the agent can recover exact details later if needed, and compaction summaries themselves are not eligible for later compaction.
...
- **Tool calls — configured MCP tools**: rendered in the shared tool frame using the imported prefixed tool name (for example `docs__search`). Tool-call previews render the streamed JSON arguments, and text-heavy result bodies use [verbose tool rendering](#verbose-tool-rendering) so `/verbose` toggles between compact tail previews and full output. Image output is rendered directly from the tool-result content when present.
...
- Scope: shell tool-call bodies and tool-result bodies; read tool-call bodies and tool-result bodies; grep tool-call bodies and tool-result bodies; edit tool-call preview bodies and edit error results; configured MCP tool-call bodies and text result bodies. Successful edit results stay compact in both modes. Todo tool results ignore `/verbose` and always show the full current list.
...
2. Loads global user settings from `~/.config/mini-coder/settings.json`.
3. If the current working directory is inside a git repo, loads `<git-root>/.mini-coder/settings.json` as a repo-local read-only overlay.
   - Missing or invalid content in either settings file is treated as "no settings from that file" rather than a fatal startup error.
   - Repo-local values win on collisions.
   - `customProviders` is merged by provider `name` instead of replacing the whole array. Same-name repo-local entries override global ones.
   - `mcp.servers` is merged by server `name` instead of replacing the whole array. Same-name repo-local entries override global ones.
4. Discovers user-configured custom OpenAI-compatible providers from the effective settings.
   ...
   - Invalid entries are dropped while loading settings. Duplicate custom provider names within the same settings source keep the first entry.
...
5. Loads user-configured MCP servers from the effective settings.
   ...
   - Invalid entries are dropped while loading settings. Duplicate MCP server names within the same settings source keep the first entry.
   ...
   - Servers whose effective definition comes from the repo-local overlay stay read-only. `/mcp` can toggle them during the current app run, but only globally configured servers persist new enabled states back to `~/.config/mini-coder/settings.json`.
...
6. Selects the startup model, effort, reasoning visibility, and verbose mode from those effective settings when available.
...
7. Loads AGENTS.md files and skills.
8. Selects the launch mode:
   - Interactive TUI when `stdin` and `stdout` are both TTYs and `-p` was not provided.
   - Headless one-shot mode otherwise.
9. In interactive mode, renders the UI with an empty conversation log, a minimal `mini-coder` empty-state banner (showing the packaged version when available, otherwise a simple dev label), and the status bar populated.
10. Starts a new session only when a prompt is actually submitted (no 0-message sessions in the DB).
...
mini-coder persists global user defaults in `~/.config/mini-coder/settings.json`. When launched inside a git repo, it also reads `<git-root>/.mini-coder/settings.json` as a repo-local read-only overlay for that app run.

### Stored settings

The global settings file and the optional repo-local overlay use the same JSON shape.
...
- Global defaults are persisted in `~/.config/mini-coder/settings.json`.
- When launched inside a git repo, mini-coder also loads `<git-root>/.mini-coder/settings.json` as a repo-local read-only overlay for that app run.
- The effective settings are built by loading the global file first and then overlaying the repo-local file. Repo-local values win on collisions.
- `customProviders` is merged by provider `name` instead of replacing the whole array. Same-name repo-local entries override global ones.
- `mcp.servers` is merged by server `name` instead of replacing the whole array. Same-name repo-local entries override global ones.
- `/model`, `/effort`, `/reasoning`, and `/verbose` persist their new values immediately to the global settings file only.
...
- Invalid or missing content in either settings file is treated as "no settings from that file" rather than a fatal startup error.
...
- The effective `customProviders` list is an optional array of user-configured OpenAI-compatible endpoints, typically local model servers.
...
- The effective `mcp.servers` list is an optional array of user-configured Streamable HTTP MCP endpoints.
...
- MCP server definitions are loaded at startup. `/mcp` can toggle configured servers on or off for future turns during the current app run. For servers defined only in global settings, that on/off state is persisted back to `~/.config/mini-coder/settings.json`. Servers whose effective definition comes from the repo-local overlay are read-only and their toggles last only until the app exits. Turning a server on reconnects it on demand; turning it off disconnects it. There is still no interactive slash command to add, remove, or reload server definitions.
...
CREATE TABLE session_compactions (
  id             INTEGER PRIMARY KEY AUTOINCREMENT,
  session_id     TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,
  message_end_id INTEGER NOT NULL,  -- last raw message-row id covered by this summary
  summary_data   TEXT NOT NULL,     -- JSON-serialized synthetic user summary message
  usage_data     TEXT,              -- JSON-serialized assistant usage for the hidden summary call
  created_at     INTEGER NOT NULL   -- unix ms
);

CREATE INDEX idx_session_compactions_session ON session_compactions(session_id, message_end_id);
...
**Compaction summaries are stored separately from the raw transcript**: automatic context compaction never rewrites or deletes old conversational rows just to save context. Instead, each compaction stores one synthetic user-summary message in `session_compactions`, the raw message-row boundary it replaces, and the hidden summarization-call usage for stats/accounting. Future model context is rebuilt from those stored summaries followed by the remaining uncompacted raw message tail, while the full original transcript stays available in `messages` for exact recovery.
...
**Cumulative stats are computed, not stored**: the status bar's cumulative `in`, `out`, and `$cost` values are computed on session load by summing `usage` from assistant messages in `messages` plus any persisted hidden-summary usage in `session_compactions`. The message/compaction count per session is small (hundreds), so this is fast. No separate counters to keep in sync. During an active session, a running in-memory accumulator is updated after each assistant message or hidden compaction-summary call to avoid re-scanning. `context%/window` is separate: it is estimated from the current model-visible history for the next request rather than stored cumulatively.
...
| Compact context | `INSERT INTO session_compactions` with the synthetic summary message, hidden summary-call usage, and the last raw message-row id it replaces                                                                                                                                                                     |
| Undo            | `DELETE FROM messages WHERE session_id = ? AND turn = (SELECT MAX(turn) FROM messages WHERE session_id = ?)` plus `DELETE FROM session_compactions WHERE session_id = ? AND message_end_id >= <first row id in the deleted turn>`                                                                                |
| Fork            | `INSERT INTO sessions` (new id, `forked_from` set), then `INSERT INTO messages SELECT ... FROM messages WHERE session_id = ?` (copy all, new session_id), then copy `session_compactions` with each `message_end_id` remapped onto the forked message rows, then append a UI-only fork notice with `turn = NULL` |
| List sessions   | `SELECT * FROM sessions WHERE cwd = ? ORDER BY updated_at DESC`                                                                                                                                                                                                                                                  |
| Load session    | `SELECT data FROM messages WHERE session_id = ? ORDER BY id` → parse JSON → persisted message log (`pi-ai Message[]` + internal UI messages); separately load `session_compactions` to rebuild the compacted model-visible history and add hidden summary-call usage to cumulative stats                         |
| Delete session  | `DELETE FROM sessions WHERE id = ?` (cascade deletes messages and compaction summaries)                                                                                                                                                                                                                          |
